...
* Code that can be called from external C/C++, from python, and/or from go-lang
* Code creates new or updates an existing file under `/proc` folder e.g.: `/proc/path/to/new-file`
* Path/file under /proc folder may not exist and must be created
...
* Unit test(s) to validate read from existent/nonexistent path without a crash
